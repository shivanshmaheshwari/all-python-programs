...
def add_reminder_title(update, context):
    ...
    json_add_reminder_info(update.message.from_user.id,
...
def add_reminder_info(update, context):
    ...
    context.job_queue.run_once(
        notify,
        seconds,
        context=[user, reminder_title, reminder_date,
                 reminder_time, reminder_info],
    )
...
def delete_reminder(update, context):
    with open("data.json", "r+") as file:
        content = json.load(file)
        content = content["users"][str(
            update.message.from_user.id)]["reminders"]
...
def main():
    BOT_TOKEN = "5150531747:AAHMA6zw5SZlrq8gCw32ZV30HZ9MnZELzFo"
    ...
    change_utc_handler = ConversationHandler(
        entry_points=[MessageHandler(
            Filters.regex("Change UTC"), request_utc)],
        states={
            ADD_UTC: [MessageHandler(Filters.text, add_utc)],
        },
    ...
    dp.add_handler(MessageHandler(
        Filters.regex("All Reminders"), all_reminders))

    add_reminder_handler = ConversationHandler(
        entry_points=[MessageHandler(
            Filters.regex("Add Reminder"), add_reminder)],
        states={
            REMINDER_TITLE: [
                MessageHandler(
                    Filters.text & ~Filters.regex(
                        "/cancel"), add_reminder_title
                )
            ],
            REMINDER_DATE: [
                MessageHandler(
    ...
    delete_reminder_handler = ConversationHandler(
        entry_points=[MessageHandler(Filters.regex(
            "Delete History"), delete_reminder)],
        states={
            DELETE_REMINDER_INDEX: [
                MessageHandler(
                    Filters.text & ~Filters.regex(
                        "/cancel"), delete_reminder_status
                )
            ]
        },
